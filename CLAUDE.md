...
```bash
# REQUIRED: server_id and ingest_endpoint must be provided
# The server_id should be generated by Laravel and stored in the database BEFORE deployment

# Development (no mTLS):
ansible-playbook -i inventory.yml ansible/nodepulse/deploy.yml \
  -e "server_id=abc-123-def-456" \
  -e "ingest_endpoint=https://dashboard.example.com" \
  -e "tls_enabled=false"
...
ansible-playbook -i inventory.yml ansible/nodepulse/deploy.yml \
  ...
  -e "ingest_endpoint=https://dashboard.example.com" \
  -e "tls_enabled=true" \
  -e "ca_cert=$(cat ca.crt)" \
  -e "client_cert=$(cat client.crt)" \
  -e "client_key=$(cat client.key)"

# Deploy only specific components:
ansible-playbook -i inventory.yml ansible/nodepulse/deploy.yml --tags "nodepulse" \
  -e "server_id=abc-123-def-456" -e "ingest_endpoint=https://dashboard.example.com"
...
- **Idempotent** - Safe to run multiple times (server_id is now passed as a variable)
...
**IMPORTANT:** The `server_id` must be generated by the Laravel application and saved to the database **before** running the Ansible playbook. This ensures the server_id in the database matches the server_id on the agent, preventing 403 errors.
...
- if you need to check anything in production server, give me a debug script, and I will use scp to upload to production server and run it.
