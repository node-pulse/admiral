#!/bin/bash
# NodePulse Admiral - Local Test Deployment Script
#
# ⚠️  FOR LOCAL TESTING ONLY - Uses locally built Docker images
# ⚠️  This is a modified version of deploy.sh that skips "docker compose pull"
#
# This script:
# - Uses LOCAL Docker images (built and pushed by local_test_release.sh)
# - Does NOT pull images from ghcr.io container registry
# - Otherwise identical to deploy.sh (same configuration, setup, etc.)
# - Configures production environment variables (APP_ENV=production, hardcoded)
# - Creates initial admin user
#
# This script is idempotent and can be run multiple times safely
#
# Usage: Called automatically by local_test_release.sh
# Or manually: sudo ./deploy_test.sh (if .env already exists)
...
echo -e "${BLUE}================================================${NC}"
echo -e "${BLUE}Node Pulse Admiral - Local Test Deployment${NC}"
echo -e "${BLUE}================================================${NC}"
echo ""
echo -e "${CYAN}Using locally built Docker images (no ghcr.io pull)${NC}"
echo ""
...
# =============================================================================
# Pull Docker images - DISABLED FOR LOCAL TESTING
# =============================================================================
# This section is disabled for local testing deployments
# We use locally built images instead of pulling from ghcr.io
#
# echo -e "${BLUE}================================================${NC}"
...
echo -e "${BLUE}================================================${NC}"
echo -e "${BLUE}Using Local Docker Images${NC}"
echo -e "${BLUE}================================================${NC}"
echo ""
echo -e "${GREEN}✓ Skipping docker compose pull (using local images)${NC}"
echo ""
