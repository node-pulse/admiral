...
const MASTER_KEY = process.env.RAILS_MASTER_KEY || 'e78e93131740cda850f1b0fe941933e031d3a643bbba53e8f1547204cb8b4da5';
...
function decrypt(encryptedData) {
  try {
    ...
    // Create decipher
    const decipher = crypto.createDecipheriv('aes-256-cbc', keyBuffer, Buffer.from(payload.iv, 'base64'));
...
console.log(`SSH WebSocket server running on port ${process.env.WS_PORT || 6001}`);
...
wss.on('connection', async (ws, req) => {
  const url = req.url;
  console.log(`[WS] New connection from ${req.socket.remoteAddress}, URL: ${url}`);
  ...
  console.log(`[${sessionId}] Connection for server: ${serverId}`);
  ...
  ws.on('message', async (data) => {
    try {
      ...
      console.log(`[${sessionId}] Received message type: ${message.type}`);
...
async function handleAuth(ws, connInfo, message) {
  ...
  console.log(`[${sessionId}] ServerId: ${serverId}, Has host: ${!!message.host}`);
  ...
  try {
    ...
    if (serverId) {
      console.log(`[${sessionId}] Fetching server details from database for ${serverId}`);
      ...
      if (server.private_key_content) {
        console.log(`[${sessionId}] Encrypted key data (first 100 chars):`, server.private_key_content.substring(0, 100));
        ...
        if (privateKey && privateKey !== server.private_key_content) {
          ...
          console.log(`[${sessionId}] Failed to decrypt private key, using raw value`);
      ...
      if (!message.host || !message.username) {
        throw new Error('Server ID or SSH credentials (host, username) required');
      }
    ...
    console.log(`[${sessionId}] Connecting to ${sshConfig.username}@${sshConfig.host}:${sshConfig.port}`);
    ...
    if (sshConfig.privateKey) {
      ...
      throw new Error('No authentication method provided (need privateKey or password)');
    }
...
function handleInput(connInfo, message) {
  ...
  console.log(`[${sessionId}] Input: ${JSON.stringify(input)}`);
